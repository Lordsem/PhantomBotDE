...
public class GamesListUpdater {
    ...
    public static void update(boolean force) {
        ...
        com.gmt2001.Console.debug.println("Starte GamesListUpdater");

        if (!PhantomBot.instance().getDataStore().HasKey("settings", "", "gamesList-version")) {
            com.gmt2001.Console.debug.println("Version nicht gesetzt, Initialisierung auf 0");
        ...
        if (!PhantomBot.instance().getDataStore().HasKey("settings", "", "gamesList-lastCheck")) {
            com.gmt2001.Console.debug.println("Last Check nicht gesetzt, Initialisierung auf Unix Epoch");
        ...
        com.gmt2001.Console.debug.println("Letztes Update: " + cal.toString());

        if (cal.getTime().after(new Date())) {
            com.gmt2001.Console.debug.println("Update wird übersprungen, Intervall ist noch nicht abgelaufen...");
            return;
        }
        ...
        if (!response.success) {
            com.gmt2001.Console.debug.println("Aktualisierung wird übersprungen, Anfrage fehlgeschlagen...");
        ...
        if (myVersion >= jso.getInt("version")) {
            com.gmt2001.Console.debug.println("Update wird übersprungen, aktuell Up-to-Date...");
            return;
        }
        ...
        try {
            if (myVersion > 0) {
                com.gmt2001.Console.debug.println("Lade aktuelle gamesList.txt...");
                List<String> odata = Files.readAllLines(Paths.get("./web/panel/js/utils/gamesList.txt"));
                data.addAll(odata);
                com.gmt2001.Console.debug.println("Geladene " + data.size() + " Einträge");
            }
        ...
        com.gmt2001.Console.debug.println("Ausführen von Updates, um Version zu erreichen " + jso.getInt("version") + "...");

        boolean fullUpdate = false;
        while (myVersion < jso.getInt("version")) {
            int nextVersion = myVersion + 1;
            com.gmt2001.Console.debug.println("Aktuelle Version ist " + myVersion + ", Update auf Version " + nextVersion + "...");

            JSONArray indexesToUpdate;
            if (jso.getJSONObject("index_changes").has("" + nextVersion)) {
                com.gmt2001.Console.debug.println("Änderungsliste für Version gefunden " + nextVersion);
                indexesToUpdate = jso.getJSONObject("index_changes").getJSONArray("" + nextVersion);
            } else {
                com.gmt2001.Console.debug.println("Keine Änderungsliste für Version " + nextVersion + ", führe vollständiges Update aus...");
                indexesToUpdate = jso.getJSONArray("indexes");
                fullUpdate = true;
                data.clear();
            }

            com.gmt2001.Console.debug.println("Verarbeiten von Indexen...");
            ...
            if (jso.getJSONObject("deletes").has("" + nextVersion)) {
                com.gmt2001.Console.debug.println("Gefundene Löschungen für Version " + nextVersion + ", wird bearbeitet...");
                DoDeletes(data, jso.getJSONObject("deletes").getJSONArray("" + nextVersion));
            }

            if (fullUpdate) {
                com.gmt2001.Console.debug.println("Vollständiges Update abgeschlossen, Springen zur Version " + jso.getInt("version") + "...");
            ...
            try {
                com.gmt2001.Console.debug.println("Schreibe gamesList.txt Version " + myVersion + "...");
                Files.write(Paths.get("./web/panel/js/utils/gamesList.txt"), data);
                com.gmt2001.Console.debug.println("Gespeicherte " + data.size() + " Einträge");
            } catch (IOException ex) {
                com.gmt2001.Console.err.printStackTrace(ex);
                return;
            }
        ...
        com.gmt2001.Console.debug.println("Update der Kategorieliste abgeschlossen, jetzt in der Version " + myVersion);
    }
    ...
    private static void UpdateFromIndex(List<String> data, int index) {
        ...
        if (!response.success) {
            com.gmt2001.Console.debug.println("Überspringen des Indexes " + index + ", Anfrage fehlgeschlagen...");
            com.gmt2001.Console.debug.println(response.toString());
            return;
        }
        ...
        for (int i = 0; i < jsa.length(); i++) {
            com.gmt2001.Console.debug.println("Kategorie verarbeiten \"" + jsa.getJSONObject(i).getString("name") + "\"...");
            if (jsa.getJSONObject(i).has("old_names")) {
                ...
                for (int k = 0; k < old_names.length(); k++) {
                    if (data.contains(old_names.getString(k))) {
                        com.gmt2001.Console.debug.println("Einen alten Namen \"" + old_names.getString(k) + "\" gefunden, entferne...");
    ...
    private static void DoDeletes(List<String> data, JSONArray deletes) {
        for (int i = 0; i < deletes.length(); i++) {
            if (data.contains(deletes.getString(i))) {
                com.gmt2001.Console.debug.println("Lösche Kategorie \"" + deletes.getString(i) + "\"...");
